...
//
// ┌─────────────────────────────────────────────────────────────────────┐
// │ CATATAN PERBAIKAN (versi lama file ini SYNTAX ERROR):               │
// │ sebelumnya barisnya cuma `const SUPABASE_URL =` tanpa nilai apa pun │
// │ — itu bikin SELURUH file gagal di-parse, jadi `sb` gak pernah ada   │
// │ dan semua tombol test diam. Nilai WAJIB berupa string dalam tanda   │
// │ kutip, contoh: const SUPABASE_URL = "https://abcd1234.supabase.co"; │
// └─────────────────────────────────────────────────────────────────────┘

const SUPABASE_URL = "ISI_URL_SUPABASE_DISINI";        // <- ganti, contoh: "https://abcd1234.supabase.co"
const SUPABASE_ANON_KEY = "ISI_ANON_KEY_SUPABASE_DISINI"; // <- ganti dengan anon public key (mulai "eyJ..." atau "sb_publishable_...")
...
// = ...`), itu gampang ketuker mana namespace SDK, mana client hasil
// createClient(). Buat ngindarin itu total, namespace SDK dipanggil
// eksplisit lewat `window.supabaseSdk` (di-set di bawah), dan client hasil
// createClient() dikasih nama BEDA: `sb`.
window.supabaseSdk = window.supabase; // simpan referensi SDK sebelum ketiban

const sb = (CONFIG_LOOKS_UNSET || !window.supabaseSdk || !window.supabaseSdk.createClient)
  ? null
